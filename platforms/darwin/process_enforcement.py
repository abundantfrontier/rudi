...
class DarwinProcessEnforcement(ProcessEnforcement):
    ...
    def run_command(self, agent_id: str, command: List[str], cwd: str = None) -> Dict[str, Any]:
        ...
        Note: For a production R.U.D.I., this would use 'sandbox-exec' 
        or similar Darwin-specific sandboxing primitives.
        """
        cmd_str = " ".join(command)
        if self.check_permission(agent_id, "process:execute", {"command": cmd_str}):
